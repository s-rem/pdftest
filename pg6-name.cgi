...
sub Page_add_new_nenga {
    ...
    $tables->show($page_object , 55 * 2.8356 , 286 * 2.8356 , 'tl');
    ...
    $titles = Text(sprintf("%s", $person_info->{'reg_number'}),$s_reg_num);
    $titles->show($page_object , 418 , 235, 'tl');

    $titles = Text(sprintf("%s", $person_info->{'real_name'}),$s_reg_name1);
    $titles->show($page_object , 350, 272, 'lml');

    $titles = Text(sprintf("%s", $person_info->{'reg_number'}),$s_reg_num);
    $titles->show($page_object , 103 , 235, 'tl');

    my $name2 = $person_info->{'badge_name'};
    
    if(($name2_1, $name2_2, $name2_3) = ($name2 =~/(.+)<!>(.*)<!>(.*)/)){
    $tables=Block('V', [
        Paragraph(
        Text([
            sprintf("%s",$name2_1),
        ...
        PStyle(size => 193 , align => 'm', linefeed => 25, preskip => 12.5, postskip => 12.5)),
        ], $bs_confidential);
    } elsif(($name2_1, $name2_2) = ($name2 =~/(.+)<!>(.*)/)){
    $tables=Block('V', [
        Paragraph(
        Text([
            sprintf("%s",$name2_1),
    ...
    $tables=Block('V', [
        Paragraph(
            Text(sprintf("%s",$name2), $s_reg_name2),
            PStyle(size => 193 , align => 'm', linefeed => 25, preskip => 12.5, postskip => 12.5)),
        ], $bs_confidential);
    }    
    $tables->show($page_object , 62 , 280, 'lm');
